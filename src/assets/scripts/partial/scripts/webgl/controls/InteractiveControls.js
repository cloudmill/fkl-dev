...
export default class InteractiveControls extends EventEmitter {
	...
	resize(x, y, width, height) {
		if (x || y || width || height) {
			...
			this.rect = { x: 0, y: 0, width: $("#particle_photo").width() * 1.2, height: $("#particle_photo").height() };
		}
		else {
	...
	onMove(e) {
		...
		const touch = {
		  x: t.clientX - $("#particle_photo").offset().left - $("#particle_photo").parent().offset().left,
      y: t.clientY - $("#particle_photo").offset().top - $("#particle_photo").parent().offset().top - 150
		};
